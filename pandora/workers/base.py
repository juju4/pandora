...
class BaseWorker(multiprocessing.Process):
        ...
        **options,
        ...
        try:
            self.redis.xgroup_create(
                name="tasks_queue", groupname=self.module, mkstream=True
            )
            ...
            if self.disabled:
                self.logger.critical(
                    f"General error, unable to initialize the workers for {module}."
                )
    ...
    def _read_stream(self) -> Tuple[str, List[str], Optional[str]]:
        ...
        return (
            values["task_uuid"],
            json.loads(values["disabled_workers"])
            if values.get("disabled_workers")
            else [],
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                self.logger.critical(
                    f"unable to reach redis socket, shutting down : {e}"
                )
            except BaseException as e:
                self.logger.critical(
                    f"unknown error with current task : {repr(e)}\n{traceback.format_exc()}"
                )
