    ...
    def analyse(self, task: Task, report: Report, manual_trigger: bool = False):
        try:
            ...
            if "last_analysis_stats" in response["data"]["attributes"]:
                if response["data"]["attributes"]["last_analysis_stats"].get(
                    "malicious"
                ):
                    report.status = Status.ALERT
                elif response["data"]["attributes"]["last_analysis_stats"].get(
                    "suspicious"
                ):
                    report.status = Status.WARN
                elif (
                    response["data"]["attributes"]["last_analysis_stats"].get(
                        "harmless"
                    )
                    # Not sure about the infected as clean?
                    or response["data"]["attributes"]["last_analysis_stats"].get(
                        "undetected"
                    )
                ):
            ...
            if response["data"]["attributes"].get("last_analysis_results"):
                for key, detect in response["data"]["attributes"][
            ...
            report.add_details(
                "permaurl",
                f"https://www.virustotal.com/gui/file/{response['data']['id']}",
            )
            if malicious:
