...
class Irma(BaseWorker):
    ...
    async def _scan_task(self, task, report):
        ...
        config = irmacl_async.apiclient.Config(
            api_endpoint=self.apiurl, timeout=self.apitimeout
        )
        async with irmacl_async.AAPI(config=config, anonymous=True) as api:
            scan = await api.scans.scan(
                pathlib.Path(task.file.path), linger=True, force=True
            )
            ...
            report.add_details(
                'Click on "Sign in anonymously" button to reach IRMA report.', link
            )
