...
class YaraWorker(BaseWorker):
    ...
    needs_external: List[
        ...
        []
    )  # list of filenames, used for children classes with yara files requiring external variables
    ...
    def rules(self) -> yara.Rules:
        ...
        yara_files = [
            ...
            if y_file.name not in self.needs_external
        ]
        ...
        rules = yara.compile(
            filepaths={str(path): str(path) for path in yara_files}, includes=True
        )
    ...
    def analyse(self, task: Task, report: Report, manual_trigger: bool = False):
        ...
        matches = [
            str(_match)
            for _match in self.rules.match(data=task.file.data.getvalue())
            if _match
        ]
        if matches:
