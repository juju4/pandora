...
class YaraSignatureBaseWorker(YaraWorker):
    ...
    needs_external = ["generic_anomalies.yar", "general_cloaking.yar", "thor_inverse_matches.yar", "yara_mixed_ext_vars.yar"]
    ...
    def rules_with_external_vars(self, filename: str, filepath: str, filetype: str, owner: str) -> yara.Rules:
        ...
        rules = yara.compile(
            ...
            includes=True,
            externals={"filename": filename, "filepath": filepath, "extension": extension, "filetype": filetype, "owner": owner},
        )
        return rules
    ...
    def analyse(self, task: Task, report: Report, manual_trigger: bool = False):
        ...
        rules_external = self.rules_with_external_vars(
            filename=task.file.original_filename, filepath=task.file.original_filename, filetype=filetype, owner=owner
        )
