...
class Pandora:
    def __init__(self) -> None:
        ...
        self.redis_pool_cache_bytes: ConnectionPool = ConnectionPool(
            connection_class=UnixDomainSocketConnection, path=get_socket_path("cache")
        )
    ...
    def get_tasks(
        ...
        user: User,
        *,
        first_date: Union[datetime, int, float, str] = 0,
        last_date: Union[datetime, int, float, str] = "+Inf",
    ):
    ...
    def add_seed(
        self, task: Task, time: str, seed: Optional[str] = None
    ) -> Tuple[str, int]:
