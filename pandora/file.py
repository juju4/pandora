...
def html_to_pdf(source: Union[str, bytes, Path], dest: str) -> None:
    ...
    if isinstance(source, str):
        html = HTML(
            ...
            if get_config("generic", "weasyprint_fetch_ressources")
            else disable_fetch_weasyprint,
        )
...
def office_to_pdf(source: Union[Path, bytes], dest: str) -> None:
    ...
    try:
        ...
        raise Unsupported(
            f"The Office document is probably password protected, this feature isn't supported yet - Error message: {e}."
        )
...
class File:
    ...
    DATA_CHARSETS: List[str] = ["utf8", "latin1", "ascii"]
        ...
        *,
        ...
        if self.deleted:
            ...
            if not md5 or not sha1 or not sha256:
                raise Exception(
                    f"The hashes should have been initialized. md5: {md5}, sha1: {sha1}, sha256: {sha256}"
                )
            if not size:
        ...
        if self.eml_data:
            ...
            if "body" in self.eml_data:
                for i, body_part in enumerate(self.eml_data["body"]):
                    if "content_type" in body_part and self.MIME_TYPE_EQUAL.get(
                        body_part["content_type"]
                    ):
                        body_part_type = self.MIME_TYPE_EQUAL[
                            body_part["content_type"]
                        ][0]
                        if body_part_type == "HTM":
                            html_to_pdf(
                                body_part["content"], f"{self.path}_body_{i}.pdf"
                            )
                        elif body_part_type == "TXT":
                            converter = dirty_load_unoconverter().UnoConverter()
                            converter.convert(
                                indata=body_part["content"].encode(),
                                outpath=f"{self.path}_body_{i}.pdf",
                            )
                        else:
    ...
    def make_previews(self) -> None:
        ...
        for i, p in enumerate(to_convert):
            ...
            if doc.needs_pass:
                raise Unsupported(
                    "The PDF is password protected, this feature isn't supported yet."
                )
    ...
    def sha1(self) -> str:
        ...
        if self._sha1 is None and self.data:
            self._sha1 = (
                hashlib.sha1(self.data.getvalue()).hexdigest()
                if self.data is not None
                else None
            )
    ...
    def sha256(self) -> str:
        ...
        if self._sha256 is None and self.data:
            self._sha256 = (
                hashlib.sha256(self.data.getvalue()).hexdigest()
                if self.data is not None
                else None
            )
        ...
        try:
            if self.is_html or self.is_eml or self.is_txt:
                ...
                return textract.process(
                    self.path, extension=self._extension_for_textract
    ...
    def text_preview(self) -> BytesIO:
        ...
        try:
            ...
            out = Image.new(
                "L",
                (
                    text_width if text_width < max_width else max_width,
                    text_height if text_height < max_height else max_height,
                ),
                255,
            )
    ...
    def msg_data(self) -> Optional[Message]:
        ...
        assert isinstance(
            msg, Message
        ), f"msg file must be a message, other formats are not supported yet. Type: {type(msg)}"
        return msg
    ...
    def metadata(self) -> Dict[str, str]:
        ...
        with exiftool.ExifTool() as et:
            for key, value in et.get_metadata_batch([str(self.path)])[0].items():
                if any(
                    key.lower().startswith(word)
                    for word in ("sourcefile", "exiftool:", "file:")
                ):
                    continue
                key = key.split(":")[-1]
    ...
    def misp_export(
        self,
    ) -> Optional[Tuple[FileObject, Optional[MISPObject], Optional[List[MISPObject]]]]:
        try:
            # Currently only extract indicators from binary files (PE, ELF, MachO)
            return make_binary_objects(
                pseudofile=self.data, filename=self.original_filename
            )
        except Exception:
            traceback.print_exc()
        return None
