...
def html_to_pdf(source: Union[str, bytes, Path], dest: str) -> None:
    ...
    if isinstance(source, str):
        ...
        html = HTML(
            file_obj=BytesIO(source), url_fetcher=default_url_fetcher if get_config("generic", "weasyprint_fetch_ressources") else disable_fetch_weasyprint
        )
...
class File:
    ...
    def text_preview(self) -> BytesIO:
        ...
        try:
            ...
            out = Image.new("L", (text_width if text_width < max_width else max_width, text_height if text_height < max_height else max_height), 255)
