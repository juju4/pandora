...
def main():
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Run all commands without asking.",
    )
    ...
    keep_going(args.yes)
    run_command(
        f'poetry run {(Path("tools") / "validate_config_files.py").as_posix()} --check'
    )

    print("* Update configuration files.")
    keep_going(args.yes)
    run_command(
        f'poetry run {(Path("tools") / "validate_config_files.py").as_posix()} --update'
    )
