...
class WorkersManager(AbstractManager):
    ...
    def _init_worker(
        self,
        module_name: str,
        worker_conf: Dict[str, Dict[str, str]],
        restart: bool = False,
    ) -> List[BaseWorker]:
        """
        ...
        options = {
            key: value
            for key, value in worker_conf["settings"].items()
        ...
        for i in range(1, replicas + 1):
            try:
                ...
                raise AssertionError(
                    f"missing mandatory key {key} for worker in config"
                )
            else:
    ...
    def _manager(self):
        ...
        for worker in self._workers:
            ...
            new_worker = self._init_worker(
                module_name, worker_conf=workers()[module_name], restart=True
            )[0]
