...
class IMAPFetcher(AbstractManager):
    ...
    def _prepare_reply(self, initial_message: EmailMessage, permaurl: str) -> Optional[EmailMessage]:
        ...
        body = body.format(recipient=recipient, permaurl=permaurl, sender=msg["From"].addresses[0].display_name)
    ...
    def _imap_fetcher(self):
        ...
        with IMAPClient(host=self.imap_server, ssl_context=ssl_context) as client:
            ...
            for uid, message_data in client.fetch(messages, "RFC822").items():
                ...
                new_task = Task.new_task(user=user, sample=BytesIO(email_message.as_bytes()), disabled_workers=[], filename=f'{email_message["subject"]}.eml')
