...
upload_parser.add_argument("file", location="files", type=FileStorage, required=True, help="The file you want to analyze")
upload_parser.add_argument(
    "validity", type=int, required=False, location="args", help="Number of seconds the seed will be valid (0 means forever, empty doesn't create a seed)."
)
upload_parser.add_argument(
    "password", type=str, required=False, location="args", help="User defined password to decrypt the file (generally used for archives)."
)
...
class ApiSubmit(Resource):
    ...
    def post(self):
        ...
        try:
            task = Task.new_task(
                flask_login.current_user, sample=file_bytes, filename=submitted_file.filename, password=password, disabled_workers=disabled_workers
            )
        ...
        if args.get("validity") is not None:
            ...
            return {"success": True, "taskId": task.uuid, "seed": seed, "lifetime": expire, "link": link}
        link = url_for("api_analysis", task_id=task.uuid)
        return {"success": True, "taskId": task.uuid, "link": link}


status_parser = api.parser()
status_parser.add_argument("task_id", required=True, location="args", help="The id of the task you'd like to get the status of")
status_parser.add_argument("seed", required=False, location="args", help="The seed of the task you'd like to get the status of")
status_parser.add_argument(
    "details", type=int, required=False, location="args", help="Do you want details about the report status of every worker ? If yes, print 1, else print 0"
)
...
worker_parser.add_argument("task_id", required=True, location="args", help="The id of the task you'd like to get the status of")
worker_parser.add_argument("seed", required=False, location="args", help="The seed of the task you'd like to get the status of")
worker_parser.add_argument(
    "all_workers", type=int, required=False, location="args", help="Do you want the details of every workers ? If yes, print 1, else print 0"
)
worker_parser.add_argument("worker_name", required=False, location="args", help="The name of the worker you want to get the report of")
worker_parser.add_argument(
    "details", type=int, required=False, location="args", help="Do you want the details of the worker status ? If yes, print 1, else print 0"
)
...
@api.route("/task-action/<task_id>/<action>", "/task-action/<task_id>/seed-<seed>/<action>", methods=["POST"], strict_slashes=False, doc=False)
    ...
    def post(self, task_id, action, seed=None):
        ...
        if action == "refresh" and flask_login.current_user.role.can(Action.refresh_analysis):
            ...
            return {
                "success": True,
                "task": task.to_dict,
                "workers_done": task.workers_done,
                "seed": seed,
                "workers_status": task.workers_status,
        ...
        if action == "notify" and flask_login.current_user.role.can(Action.notify_cert):
            ...
            message = "\n".join(
                [f'-- Message from {data["email"]} --', f'-- Page {url_for("api_analysis", task_id=task.uuid, seed=seed)} --', "", data["message"]]
            )
            sent = Mail.send(
...
@api.route("/api/search/<query>", "/api/search/<query>/<int:days>", methods=["GET"], strict_slashes=False)
@api.doc(
    description="Search a task by hash or name. The 'days' parameter (10 by default) is there to limit how far in the past we go for the search",
    security="apikey",
)
class ApiSearch(Resource):
    ...
    def get(self, query: str, days: int = 10):
        ...
        for task in pandora.get_tasks(user=flask_login.current_user, first_date=first_date):
            if query in [task.file.md5, task.file.sha1, task.file.sha256] or [
                name for name in [task.file.original_filename, task.file.path.name] if query in name
            ]:
...
def _normalize_day(year: Optional[Union[str, int]], month: Optional[Union[str, int]], day: Optional[Union[str, int]]) -> Tuple[datetime, datetime]:
    if day:
        if not month:
...
@api.route("/api/stats/submit/year", "/api/stats/submit/year/<string:year>", methods=["GET"], strict_slashes=False)
...
class ApiSubmitStatsYear(Resource):
    ...
    def get(self, year: Optional[str] = None):
        ...
        to_return: Dict[str, Any] = {"date_start": first_date.date().isoformat(), "date_end": last_date.date().isoformat(), "sub_months": []}
...
@api.route(
    "/api/stats/submit/month",
    "/api/stats/submit/month/<string:month>",
    "/api/stats/submit/month/<string:month>/<string:year>",
    methods=["GET"],
...
class ApiSubmitStatsMonth(Resource):
    ...
    def get(self, year: Optional[str] = None, month: Optional[str] = None):
        ...
        to_return: Dict[str, Any] = {"date_start": first_date.date().isoformat(), "date_end": last_date.date().isoformat(), "sub_weeks": []}
...
@api.route(
    "/api/stats/submit/week",
    "/api/stats/submit/week/<string:week>",
    "/api/stats/submit/week/<string:week>/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
...
class ApiSubmitStatsWeek(Resource):
    ...
    def get(self, year: Optional[str] = None, week: Optional[str] = None):
        ...
        to_return: Dict[str, Any] = {"date_start": first_date.date().isoformat(), "date_end": last_date.date().isoformat(), "sub_days": []}
        for first, last in _intervals(rrule.DAILY, first_date, last_date):
            tasks_count = pandora.storage.count_tasks(first_date=first.timestamp(), last_date=last.timestamp())
...
@api.route(
    "/api/stats/submit/day",
    "/api/stats/submit/day/<string:day>",
    "/api/stats/submit/day/<string:day>/<string:month>",
    "/api/stats/submit/day/<string:day>/<string:month>/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
...
class ApiSubmitStatsDay(Resource):
    ...
    def get(self, year: Optional[str] = None, month: Optional[str] = None, day: Optional[str] = None):
        ...
        to_return: Dict[str, Any] = {"date_start": first_date.date().isoformat(), "date_end": last_date.date().isoformat(), "sub_hours": []}
...
def _stats(intervals: List[Tuple[datetime, datetime]]) -> Dict:
    to_return: Dict[str, Any] = {"date_start": intervals[0][0].date().isoformat(), "date_end": intervals[-1][1].date().isoformat()}
...
@api.route("/api/stats/year", "/api/stats/year/<string:year>", methods=["GET"], strict_slashes=False)
...
@api.route("/api/stats/month", "/api/stats/month/<string:month>", "/api/stats/month/<string:month>/<string:year>", methods=["GET"], strict_slashes=False)
...
@api.route("/api/stats/week", "/api/stats/week/<string:week>", "/api/stats/week/<string:week>/<string:year>", methods=["GET"], strict_slashes=False)
...
@api.route(
    ...
    "/api/stats/day/<string:day>/<string:month>/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
