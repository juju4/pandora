...
class AuthToken(Resource):
    ...
    def get(self):
        ...
        if username in self.users_table and check_password_hash(
            self.users_table[username]["password"], password
        ):
...
class ApiSubmit(Resource):
    ...
    def post(self):
        ...
        disabled_workers = (
            request.form["workersDisabled"].split(",")
            if request.form.get("workersDisabled")
            else []
        )
        password = (
            request.form["password"]
            if request.form.get("password")
            else args.get("password")
        )
        try:
            task = Task.new_task(
...
@api.route(
    "/task-action/<task_id>/<action>",
    "/task-action/<task_id>/seed-<seed>/<action>",
    methods=["POST"],
...
class ApiTaskAction(Resource):
    ...
    def post(self, task_id, action, seed=None):
        ...
        if action == "refresh" and flask_login.current_user.role.can(
        ...
        if action == "share" and flask_login.current_user.role.can(
        ...
        if action == "rescan" and flask_login.current_user.role.can(Action.rescan_file):
            ...
            if pubsub_config["enabled"] and pubsub_config["channel_submission"]:
                ...
                if misp_file_objs:
                    pandora.publish_on_channel(
                        pubsub_config["channel_submission"], misp_file_objs[0].to_json()
                    )
...
class ApiSearch(Resource):
    ...
    def get(self, query: str, days: int = 10):
        ...
        for task in pandora.get_tasks(
            user=flask_login.current_user, first_date=first_date
        ):
...
def _intervals(
    freq: int, first: datetime, last: datetime
...
def _normalize_month(
    year: Optional[Union[str, int]], month: Optional[Union[str, int]]
...
def _normalize_week(
    year: Optional[Union[str, int]], week: Optional[Union[str, int]]
) -> Tuple[datetime, datetime]:
    if week:
        if not year:
            year = datetime.now().year
...
@api.route(
    "/api/stats/submit/year",
    "/api/stats/submit/year/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
...
class ApiSubmitStatsYear(Resource):
    ...
    def get(self, year: Optional[str] = None):
        ...
        for first, last in _intervals(rrule.MONTHLY, first_date, last_date):
            tasks_count = pandora.storage.count_tasks(
                first_date=first.timestamp(), last_date=last.timestamp()
            )
...
def _stats(intervals: List[Tuple[datetime, datetime]]) -> Dict:
    to_return: Dict[str, Any] = {
        "date_start": intervals[0][0].date().isoformat(),
    ...
    for first, last in intervals:
        tasks = pandora.storage.get_tasks(
            first_date=first.timestamp(), last_date=last.timestamp()
        )
        ...
        for t in tasks:
            ...
            to_return["submit_size"]["min"] = min(
                to_return["submit_size"]["min"], task.file.size
            )
            to_return["submit_size"]["max"] = max(
                to_return["submit_size"]["max"], task.file.size
            )
    ...
    if to_return["submit"]["total"]:
        to_return["submit_size"]["avg"] = sizeof_fmt(
            to_return["submit_size"]["avg"] / to_return["submit"]["total"]
        )
        ...
        to_return["metrics"]["alert_ratio"] = (
            nb_alert / to_return["submit"]["total"] * 100
        )
...
@api.route(
    ...
    "/api/stats/month/<string:month>/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
...
@api.route(
    ...
    "/api/stats/week/<string:week>/<string:year>",
    methods=["GET"],
    strict_slashes=False,
)
