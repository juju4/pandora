...
from flask import Flask, request, session, abort, render_template, redirect, send_file, url_for, flash
...
from .helpers import get_secret_key, update_user_role, admin_required, src_request_ip, load_user_from_request, build_users_table, sri_load, sizeof_fmt
...
api = Api(title="Pandora API", description="API to query Pandora.", doc="/doc/", authorizations=authorizations, version=version("pandora"))
...
def _load_user_from_request(request):
    ...
    if user_name:
        return User(session.sid, last_ip=src_request_ip(request), name=user_name, role="admin")
    return None
...
def api_error_404(_):
    return render_template("error.html", show_project_page=get_config("generic", "show_project_page"), status=404), 404


@app.errorhandler(403)
def api_error_403(_):
    return render_template("error.html", show_project_page=get_config("generic", "show_project_page"), status=403), 403
...
def api_admin_page(error=None):
    ...
    return render_template("admin.html", show_project_page=get_config("generic", "show_project_page"), error=error_messages.get(error))
...
def api_users():
    ...
    return render_template("users.html", show_project_page=get_config("generic", "show_project_page"), users=users)
...
def api_roles():
    ...
    return render_template("roles.html", show_project_page=get_config("generic", "show_project_page"), roles=roles, api=api, api_resource=ApiRole)
...
def html_observables(task_id: str, seed: Optional[str] = None):
    ...
    assert task is not None, "analysis not found"
    update_user_role(pandora, task, seed)
    return render_template("observables_list.html", lookyloo_url=get_config("generic", "lookyloo_url"), task=task, seed=seed)
...
def api_stats():
    ...
    return render_template("stats.html", show_project_page=get_config("generic", "show_project_page"))
